...
export async function applyTryScenario(tryName: string, options: { cwd: string; filePath?: string }) {
  ...
  assert(validNames.includes(tryName), `Invalid try scenario name: ${tryName}. Valid names: ${validNames.join(', ')}`);
